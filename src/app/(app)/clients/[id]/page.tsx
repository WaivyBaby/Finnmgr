...
/* ── Build monthly revenue trend from invoices ── */
...
/* ── Edit Drawer ── */
function EditDrawer({ open, onClose, client, onSaved }: { open: boolean; onClose: () => void; client: Client; onSaved: (c: Client) => void }) {
  const [form, setForm] = useState({ name: client.name, company: client.company ?? '', email: client.email ?? '', phone: client.phone ?? '', website: client.website ?? '', industry: client.industry ?? '', status: client.status, payment_terms: client.payment_terms ?? 'Net 30', tags: client.tags ?? [], notes: client.notes ?? '' })
            ...
            <form onSubmit={submit} style={{ flex: 1, overflowY: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={{ gridColumn: '1/-1' }}>
                  ...
                  <input style={iStyle} required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                </div>
...
export default function ClientProfilePage({ params }: { params: { id: string } }) {
  ...
  const noteCards = client.notes ? client.notes.split('\n\n').filter(Boolean).reverse() : []
  ...
  return (
    <div className="page-content">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        ...
        {/* ── Profile Hero ── */}
        <div className="glass-card" style={{ padding: '24px 28px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ color: 'var(--mu2)', fontSize: 11, marginTop: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--bd)', marginBottom: 20 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)}
              style={{ padding: '10px 18px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: activeTab === t.key ? 700 : 500, color: activeTab === t.key ? '#6366f1' : 'var(--mu)', borderBottom: activeTab === t.key ? '2px solid #6366f1' : '2px solid transparent', marginBottom: -1, transition: 'all 0.15s' }}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
            ...
            {activeTab === 'overview' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                <div className="glass-card" style={{ padding: 24 }}>
                  ...
                  {[
                    ...
                    <div key={row.label} style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--mu2)', minWidth: 110 }}>{row.label}</span>
                ...
                <div className="glass-card" style={{ padding: 24 }}>
                  <h2 style={{ fontWeight: 800, fontSize: 15, color: 'var(--ink)', letterSpacing: '-0.03em', marginBottom: 16 }}>Notes</h2>
                  {noteCards.length === 0 ? (
                ...
                <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
                  ...
                  {invoices.length === 0 ? (
                    <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--mu)' }}>
                      ...
                      <button className="btn-primary" style={{ marginTop: 12 }} onClick={() => router.push('/invoices')}>Create Invoice →</button>
                    </div>
                  ) : (
            ...
            {activeTab === 'notes' && (
              <div>
                ...
                {noteCards.length === 0 ? (
                  <div className="glass-card" style={{ padding: 32, textAlign: 'center', color: 'var(--mu)' }}>
                    <p style={{ fontSize: 24, marginBottom: 10 }}>📝</p>
                    <p style={{ fontSize: 14, fontWeight: 600 }}>No notes yet</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {noteCards.map((note, i) => (
                      <div key={i} className="glass-card" style={{ padding: 18 }}>
                        <p style={{ fontSize: 13, color: 'var(--ink)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{note}</p>
                      </div>
                    ))}
                  </div>
                )}
